/**
 * Kit Learning App - Part 1: Setup & Initialization (JSON-LD Version)
 */
...
class KitApp {
  ...
  async #loadArticles() {
    ...
    try {
      ...
      // Henter ut listen trygt fra den nye @graph-strukturen, eller faller tilbake på flat JSON
  ...
  async #ensureLoadedAndScroll(articleId, hash) {
    ...
    // LATLASTING (LAZY LOADING) AV BRØDTEKST FRA INDIVIDUELL JSON-FIL:
    if (!article.text && !article.markdownContent && article.url) {
      try {
        const res = await fetch(article.url);
        if (res.ok) {
          const fullModuleData = await res.json();
          article.text = fullModuleData.text;
        }
      } catch (err) {
  ...
  #filter(resetPagination = false) {
    ...
    let result = all.filter((a) => {
      ...
      // Mapper om rollen trygt fra det dype Schema.org-objektet, med fallback til gamle felt
      ...
      const haystack = `${currentName} ${currentDesc} ${articleTags?.join(' ') ?? ''}`.toLowerCase();
  ...
  /**
   * Kit Learning App - Part 3: UI Rendering & Markdown-It Integration
   */
  #render() {
  ...
  #articleHTML(article) {
    ...
    if (isExpanded) {
      ...
      // Henter teksten direkte fra den lagrede JSON-LD strengen ("text")
      const rawMarkdown = article.text || article.body || article.markdownContent;
      const body = md && rawMarkdown ? md.render(rawMarkdown) : '';
      ...
      const nextBtn = next
        ? `<button class="next-step-btn" data-next-id="${nextId}">Next module →</button>`
        : '';
  ...
  #onSearch(raw) {
    ...
    // TERNERSKEL: Stopper søket hvis brukeren kun har tastet 1 eller 2 tegn
  ...
  async #selectModule(id, hash = '') {
    ...
    // FEDERERT INDIVIDUELL JSON-FETCH HVIS INNHOLDET MANGLER I MINNET:
    if (article && !article.text && article.url) {
      try {
        const res = await fetch(article.url);
        if (res.ok) {
          const fullModuleData = await res.json();
